...
NOTE: Your local host system Yarn version may vary from the DDEV container's Yarn version (1.22.22 at time of writing). You can use Volta or another means to pin your version while building the theme assets. Using modern versions of Yarn (v.4+) can lead to failure for packages to update, and for assets compiled for production to be formatted rather than minified. TODO: Future task to improve how the theme assets are built.
