...
class AsuDegreeRfiDegreeDetailsBlock extends BlockBase {
  ...
  public function getCacheTags() {
    ...
    return Cache::mergeTags(parent::getCacheTags(), ['degree_details_block_cache']);
  }
  ...
  public function build() {
    ...
    if (!empty($cards)) {
      ...
      if (isset($cards[0])) {
        $labelled_cards->learnMore = $cards[0];
      }
      if (isset($cards[1])) {
        $labelled_cards->apply = $cards[1];
      }
      if (isset($cards[2])) {
        $labelled_cards->visit = $cards[2];
      }
    ...
    if (!empty($cards)) {
      ...
      if (isset($cards[1])) {
        $labelled_cards->programs = $cards[1];
      }
      if (isset($cards[2])) {
        $labelled_cards->research = $cards[2];
      }
    }
    // - Bottom Row
    // Reset.
    ...
    if (!empty($cards)) {
      ...
      if (isset($cards[1])) {
        $labelled_cards->mentors = $cards[1];
      }
      if (isset($cards[2])) {
        $labelled_cards->honors = $cards[2];
      }
    }

    // Has intro text or hide setting, but no cards.
    if ($why_choose > 0) {
      // Also has cards.
      if ($why_choose > 1) {
