(function ($, Drupal, drupalSettings, once) {
  ...
  Drupal.behaviors.cardArrangementAlert = {
    attach: function (context, settings) {
      ...
      const selectCountColumn = document.querySelector(
        '[name="settings[block_form][field_card_arrangement_display]"]',
      );
      const displayOrientation = document.querySelector(
        '[name="settings[block_form][field_display_orientation]"]',
      );
      const displayDiv = document.querySelector(
        '[data-drupal-selector="edit-settings-block-form-field-display-orientation-wrapper"]',
      );
      ...
      if (ajaxNewContentDiv) {
        ...
        for (let i = 0; i < elements.length; i++) {
          ...
          for (let j = 0; j < attrs.length; j++) {
            if (
              attrs[j].value.includes('ranking') ||
              attrs[j].value.includes('based')
            ) {
              rankingOrBased = true;
      ...
      const elementRanking = document.querySelector(
        '[name*="[field_card_ranking_image_size]"]',
      );
      // Get the element to identify Image based cards
      const elementBasedCard = document.querySelector(
        '[name*="[field_loading]"]',
      );
      ...
      if (
        settings &&
        settings.columns_values &&
        Array.isArray(settings.columns_values) &&
        selectCountColumn
      ) {
      ...
      function _updateColumnsOptions() {
        if (displayOrientation && displayOrientation.value === 'horizontal') {
          const selectedValue = selectCountColumn.value;
          ...
          // Removing the selected option makes the browser silently fall back to
          // the first one, so pick the supported value explicitly instead.
          if (selectedWasRemoved) {
            selectCountColumn.value = 'two-columns';
          }
        } else {
          ...
          settings.columns_values.forEach((option) => {
            ...
            if (
              !Array.from(selectCountColumn.options).some(
                (opt) => opt.value === key,
              )
            ) {
