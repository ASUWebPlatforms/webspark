...
function getFormValidators(t) {
  return [
    (form) => {
      ...
      // Trim so a whitespace-only caption (e.g. " " or "&nbsp;") is rejected
      // as empty — an empty caption gives assistive tech nothing to announce.
      if (!form.caption.trim().length) {
        return t('A descriptive caption is required for accessibility.');
      }
    },
  ];
}
