  ...
  Drupal.behaviors.cardArrangement = {
    attach: function (context, settings) {
      ...
      for (var blockUuid in settings.asu.components.card_arrangement) {
        var arrangementData =
          settings.asu.components.card_arrangement[blockUuid];

        const isValidForCardArrangement = arrangementData.cards.length >= 1;
        const isRankingCardsOnly = arrangementData.cards.every(
          (card) => card.cardType === 'ranking',
        );
        ...
        if (isValidForCardArrangement && isRankingCardsOnly) {
          ...
          var targetEl = document.getElementById(
            'card-arrangement-' + blockUuid,
          );
          if (
            !targetEl ||
            targetEl.hasAttribute('data-react-root-initialized')
          ) {
