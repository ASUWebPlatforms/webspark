...
final class ProfileListBlock extends BlockBase implements ContainerFactoryPluginInterface {
  ...
  public function blockForm($form, FormStateInterface $form_state): array {
    ...
    foreach ($order as $position => $delta) {
      ...
      $entry['local_fields']['bio'] = [
        ...
        '#allowed_formats' => ['minimal_format'],
        '#rows' => 5,
        '#after_build' => [[static::class, 'hideTextFormatTips']],
      ];
  ...
  /**
   * After build callback to hide the text format tips/guidelines link.
   *
   * @param array $element
   *   The form element.
   ...
   */
  public static function hideTextFormatTips(array $element, FormStateInterface $form_state): array {
    if (isset($element['format'])) {
      // Hide the guidelines and help link but keep the format value
      // so CKEditor can initialize properly.
      ...
      // Hide the format selector (redundant with only one allowed format).
