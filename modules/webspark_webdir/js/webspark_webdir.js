import * as webdirUI from '@asu/app-webdir-ui';
      ...
      async function toggleWhenFullyLoaded() {
        try {
          const selectors = [
            ...
            '#campus-tree-options',
            '#expertise-tree-options',
            '#employee-type-tree-options',
            '#asurite-list-options',
          ];
  ...
  return new Promise((resolve, reject) => {
    ...
    setTimeout(() => {
      ...
      if (elementsMap.size === selectors.length) {
        ...
        reject(
          new Error(
            `Elements with selectors "${selectors}" not found within ${timeout}ms`,
          ),
        );
      }
    }, timeout);
...
jQuery.fn.extend({
  toggleFilters: function () {
    const componentType = document.querySelector(
      '[data-drupal-selector="edit-settings-block-form-field-component-type"]',
    );
    if (
      componentType?.options[componentType.selectedIndex].value ===
      'departments'
    ) {
      const defaultSort = document.querySelector(
        '[data-drupal-selector="edit-settings-block-form-field-default-sort"]',
      );
      const titleFilterBlock = document.querySelector(
        '.form-item-settings-block-form-field-filter-title-0-value',
      );
      const campusFilterBlock = document.querySelector(
        '[data-drupal-selector="edit-settings-block-form-field-filter-campus-0"]',
      );
      const expertiseFilterBlock = document.querySelector(
        '[data-drupal-selector="edit-settings-block-form-field-filter-expertise-0"]',
      );
      const employeeTypeFilterBlock = document.querySelector(
        '[data-drupal-selector="edit-settings-block-form-field-filter-employee-0"]',
      );
      const peopleListBlock = document.querySelector(
        '[data-drupal-selector="edit-settings-block-form-field-people-list-0"]',
      );
      const displaySettingsBlock = document.querySelector(
        '[data-drupal-selector="edit-settings-block-form-group-display-settings"]',
      );
      const alphaFilterBlock = document.querySelector(
        '.form-item-settings-block-form-field-webdir-disable-alpha-value',
      );
      let initAlphaFilterCheckboxState = document.querySelector(
        '[data-drupal-selector="edit-settings-block-form-field-webdir-disable-alpha-value"]',
      ).checked;
      ...
      document
        ...
        ?.addEventListener('change', function () {
          initAlphaFilterCheckboxState = document.querySelector(
            '[data-drupal-selector="edit-settings-block-form-field-webdir-disable-alpha-value"]',
          ).checked;
        });
      ...
      function show(initAlphaFilterCheckboxState) {
        ...
        document.querySelector(
          '[data-drupal-selector="edit-settings-block-form-field-webdir-disable-alpha-value"]',
        ).checked = initAlphaFilterCheckboxState;
        // Show alpha filter.
