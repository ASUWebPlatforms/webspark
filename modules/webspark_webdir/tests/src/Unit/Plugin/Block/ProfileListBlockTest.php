...
class ProfileListBlockTest extends UnitTestCase {
  ...
  /**
   * Tests the hideTextFormatTips after_build callback.
   *
   * @covers ::hideTextFormatTips
   */
  public function testHideTextFormatTips(): void {
    $element = [
      'format' => [
        'guidelines' => ['#markup' => 'Some tips'],
        'help' => ['#markup' => 'Help link'],
        'format' => [
          '#type' => 'select',
          '#options' => ['minimal_format' => 'Minimal'],
        ],
      ],
      'value' => [
        '#type' => 'textarea',
    ...
    $result = ProfileListBlock::hideTextFormatTips($element, $form_state);

    // Guidelines, help, and format selector should all be hidden.
    $this->assertFalse($result['format']['guidelines']['#access']);
    $this->assertFalse($result['format']['help']['#access']);
    $this->assertFalse($result['format']['format']['#access']);
  }

  /**
   * Tests hideTextFormatTips with missing format sub-elements.
   *
   * @covers ::hideTextFormatTips
   */
  public function testHideTextFormatTipsWithMissingElements(): void {
    // Only 'guidelines' exists, no 'help' or 'format'.
    $element = [
      'format' => [
        'guidelines' => ['#markup' => 'Tips'],
      ],
      'value' => ['#type' => 'textarea'],
    ];
    ...
    $this->assertFalse($result['format']['guidelines']['#access']);
    // Non-existent keys unchanged.
    $this->assertArrayNotHasKey('help', $result['format']);
    $this->assertArrayNotHasKey('format', $result['format']);
  }

  /**
   * Tests hideTextFormatTips when format key doesn't exist at all.
   *
   * @covers ::hideTextFormatTips
   */
  public function testHideTextFormatTipsNoFormatKey(): void {
    $element = [
      'value' => ['#type' => 'textarea'],
    ];
    ...
    $this->assertEquals(['#type' => 'textarea'], $result['value']);
  }
